  ...
  const fetchPosts = async (signal?: AbortSignal) => {
    ...
    try {
      const result = await pb.collection("posts").getList<Post>(1, 50, {
        // Zmieniono sortowanie: najpierw po dacie publikacji, potem po dacie utworzenia
        sort: "-date,-created",
        signal,
      });
      if (!signal?.aborted) {
              ...
              {posts.map((post) => (
                <tr key={post.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {/* Wyświetlanie daty publikacji, a jeśli jej brak (stare rekordy), to data utworzenia */}
