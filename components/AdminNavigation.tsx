...
const AdminNavigation: React.FC = () => {
  ...
  const [selectedItem, setSelectedItem] = useState<Partial<NavItem> | null>(null);
  ...
  const fetchNavItems = async () => {
    setLoading(true);
    try {
      const result = await pb.collection('navigation_items').getFullList<NavItem>({
        sort: 'order',
      });
      setNavItems(result);
    } catch (error) {
  ...
  return (
    <div className="container mx-auto p-4">
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={() => setSelectedItem({})} className="inline-flex items-center gap-2 bg-indigo-600 text-white px-4 py-2 rounded-md hover:bg-indigo-700">
          <Plus size={18} />
          Dodaj nową pozycję
        </button>
      </div>
        ...
        <div className="bg-white shadow-md rounded-lg overflow-hidden">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Kolejność</th>
                ...
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Rodzic</th>
              ...
              {navItems.map((item) => (
                <tr key={item.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {item.parent_id ? navItems.find(p => p.id === item.parent_id)?.name : '-'}
                  </td>
