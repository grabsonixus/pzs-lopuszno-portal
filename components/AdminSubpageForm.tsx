...
const AdminSubpageForm: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (id) {
      setLoading(true);
      setError('');
      pb.collection('subpages').getOne<Subpage>(id, { signal: controller.signal })
        .then(record => {
  ...
  return (
    <div className="container mx-auto p-4 max-w-4xl">
      <h1 className="text-2xl font-bold mb-6">{id ? 'Edytuj podstronę' : 'Dodaj nową podstronę'}</h1>
      <form onSubmit={handleSubmit} className="space-y-6 bg-white p-8 shadow-md rounded-lg">
        ...
        <div className="flex justify-end gap-4">
            <button type="button" onClick={() => navigate('/admin/subpages')} className="px-4 py-2 border rounded-md text-sm font-medium">
                Anuluj
            </button>
            <button
                type="submit"
